...
RobotBossState* RobotBossState::m_pSelf = nullptr;
RobotBossState::RobotBossState(void)
{
	m_pD3D	= nullptr;
	m_pDI	= nullptr;
	m_pTM	= nullptr;
...
RobotBossState* RobotBossState::GetInstance(void)
{
	if(m_pSelf == nullptr)
		m_pSelf = new RobotBossState;
	return m_pSelf;
}
void RobotBossState::DeleteInstance(void)
{
	if(m_pSelf != nullptr)
	{
		delete m_pSelf;
...
void RobotBossState::Enter(void)
{
	if(m_bPaused == false)
	{
		m_nEnemyCount = 0;
...
// TO DO: SET UP MESSAGES TO GET POSITIONS
	int nID =  m_pTM->LoadTexture(_T("resource/graphics/RobotBossBase.png"),0);
	m_pEnemy = m_pOF->CreateObject("CTank");
	m_pEnemy->SetImageID(nID);
	m_pEnemy->SetPosX(300);
	m_pEnemy->SetPosY(300);	
	m_pEnemy->SetWidth(64);
	m_pEnemy->SetHeight(128);

	CPlayer* player = CPlayer::GetInstance();
	m_pBoss = dynamic_cast<CTank*>(m_pEnemy);
	
	m_pBoss->SetPlayer(player);
	m_pBoss->SetRotation(0);
	m_pBoss->SetRotationRate(0.5f);
	m_pBoss->SetSight(2000);
	m_pBoss->SetVelX(10);
	m_pBoss->SetVelY(10);
	m_pBoss->SetHealth(1000);
	m_pBoss->SetMaxHealth(1000);
	m_pBoss->SetHasATurret(true);
	m_pOM->AddObject(m_pBoss);
	int nID2 =  m_pTM->LoadTexture(_T("resource/graphics/RobotBossTurret.png"),0);
	m_pTurret = m_pOF->CreateObject("CTurret");
	m_pTurret->SetImageID(nID2);
	m_pTurret->SetPosX(m_pEnemy->GetPosX());
	m_pTurret->SetPosY(m_pEnemy->GetPosY());
	m_pTurret->SetWidth(64);
	m_pTurret->SetHeight(128);

	CTurret* turret = dynamic_cast<CTurret*>(m_pTurret);
	m_pBoss->SetTurret(turret);
	turret->SetOwner(m_pEnemy);
...
bool RobotBossState::Input(void)
{
...
void RobotBossState::Update(float fDt)
{
	...
	if(!m_bPaused)
	{
		...
		if(m_pBoss->GetHealth() > 0)
		{
			...
			if(m_pBoss->GetHealth() < m_pBoss->GetMaxHealth()*0.8 && spawnsapper == false)
			{
				for(unsigned int i = 0; i < 5; i++)
				{
					CCreateEnemyMessage* msg=new CCreateEnemyMessage(MSG_CREATEENEMY, 0,m_pBoss->GetPosX()+70, m_pBoss->GetPosY()+135, rand()%3);
					CMessageSystem::GetInstance()->SndMessage(msg);
					spawnsapper = true;
				}
			}
			else if(m_pBoss->GetHealth() < m_pBoss->GetMaxHealth()*0.7 && regenhealth == false)
			{
				m_pBoss->SetHealth(m_pBoss->GetHealth() + m_pBoss->GetMaxHealth()*0.3);
				regenhealth = true;
			}
			else if(m_pBoss->GetHealth() < m_pBoss->GetMaxHealth()*0.5 && regenagain == false)
			{
				m_pBoss->SetHealth(m_pBoss->GetHealth() + m_pBoss->GetMaxHealth()*0.1);
				regenhealth = true;
				for(unsigned int i = 0; i < 5; i++)
				{
					CCreateEnemyMessage* msg=new CCreateEnemyMessage(MSG_CREATEENEMY, 3,m_pBoss->GetPosX()+70, m_pBoss->GetPosY()+135, rand()%3);
					CMessageSystem::GetInstance()->SndMessage(msg);
					regenagain = true;
				}
			}
			else if(m_pBoss->GetHealth() < m_pBoss->GetMaxHealth()*0.2 && speedboost == false)
			{
				m_pBoss->SetHealth(m_pBoss->GetHealth() + m_pBoss->GetMaxHealth()*0.2);
				speedboost = true;
				m_pBoss->SetRotationRate(0.5f);
				m_pBoss->SetSight(2000);
				m_pBoss->SetVelX(25);
...
void RobotBossState::Render(void)
{
...
void RobotBossState::MessageProc(CMessage* pMsg)
{
	RobotBossState* pSelf = RobotBossState::GetInstance();
	Camera* C=Camera::GetInstance();

...
void CGUI::Render(void)
{
	...
	if(m_pPlayer->GetDoubleDamage())
	{
		...
		m_pTM->Draw(m_nHudID, 396, 447, 1.25f,1.25f,&rect);
		m_pFont->Print(buffer, 422, 478, 1.0f, D3DCOLOR_XRGB(198,254,119));
	}
	if(m_pPlayer->GetNoReload()>0)
	{
		...
		m_pTM->Draw(m_nHudID, 270, 447, 1.25f,1.25f,&rect);
		m_pFont->Print(buffer, 295, 478, 1.0f, D3DCOLOR_XRGB(198,254,119));
	}
	if(m_pPlayer->GetInfAmmo())
	{
		...
		m_pTM->Draw(m_nHudID, 333, 447, 1.25f,1.25f,&rect);
		m_pFont->Print(buffer, 362, 478, 1.0f, D3DCOLOR_XRGB(198,254,119));
	}
	if(m_pPlayer->GetInvul())
	{
		...
		m_pTM->Draw(m_nHudID, 459, 447, 1.25f,1.25f,&rect);
		m_pFont->Print(buffer, 485, 478, 1.0f, D3DCOLOR_XRGB(198,254,119));
	}
